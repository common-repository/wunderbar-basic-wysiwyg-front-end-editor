...
( function() {
	CKEDITOR.dialog.add( 'link', function( editor ) {
		...
		// Handles the event if the user selects a Wordpress page.
		var insertWPPage = function() {
				var dialog = this.getDialog();
				var typeValue = this.getValue();
				console.log(typeValue);
				var element = this.getDialog().getContentElement( 'info', 'url' )
		...
		return {
			...
			contents: [ {
				...
				},
				{
					...
					{
						id: 'wppage',
						label: 'Use Wordpress Page',
						title: 'Use Wordpress Page',
						type: 'select',
						style: 'width : 100%;',
						items: [
							[ 'Browse...', '' ]
							],
						onChange: insertWPPage,
						onLoad: function() {
							var selectList = this;
							jQuery.getJSON(backend+'?action=wunderbar-pagelist&page=true', function(data) {
								for (var item in data) {
									selectList.add(item,data[item]);
									} 
								});
							}
						},
					...
					{
						id: 'wppost',
						label: 'Use Wordpress Post',
						title: 'Use Wordpress Post',
						type: 'select',
						style: 'width : 100%;',
						items: [
							[ 'Browse...', '' ]
						...
						onLoad: function() {
							var selectList = this;
							jQuery.getJSON(backend+'?action=wunderbar-pagelist&post=true', function(data) {
								for (var item in data) {
									selectList.add(item,data[item]);
									} 
								});
